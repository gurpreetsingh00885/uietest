...
class QuestionForm(forms.Form):
    statement = forms.CharField(max_length=1000, required=True, strip=True, widget=forms.Textarea(
        attrs={"placeholder": "Question Statement", }))


class OptionForm(forms.Form):
    value = forms.CharField(max_length=100, strip=True, widget=forms.Textarea(
        attrs={"placeholder": "Option"}))

    def __init__(self, *args, **kwargs):
        super(OptionForm, self).__init__(*args, **kwargs)
        self.fields['correct'] = forms.BooleanField(
            widget=forms.RadioSelect(choices=((self.prefix, 'Correct'),)))
...
class AssignForm(forms.Form):
    group = forms.ModelChoiceField(
        queryset=StudyGroup.objects.all(), required=True, empty_label=None)
